...
describe("getGptPayload — week overview cards", () => {
  ...
  it("puts the card's figures in front of the model", () => {
    ...
    expect(message.content).toContain(
      "- Online videos: 2 hrs tracked; down 43% vs last week",
    );
    // The trend enum is never rendered: it is the within-week adherence slope,
    // a different fact from the change beside it, and "declining" next to a
    // drop read as the week getting worse.
    expect(message.content).not.toMatch(/improving|declining|trend/i);
  });
