 ...
 */
function makePlan(
  overrides: Partial<PlanWithMeta> & { id: string },
): PlanWithMeta {
  ...
  return {
    ...
    tactics: [{ path: tacticPath } as any],
    tacticsByPath: {
      [tacticPath]: {
...
describe("crossUserEvidenceBonus via rankPlansForNextTactic", () => {
  ...
  it("gates bonus by confidence (low sample size)", () => {
    ...
    const highScore = results.find(
      (r) => r.plan.id === "high-sample",
    )!.totalScore;
