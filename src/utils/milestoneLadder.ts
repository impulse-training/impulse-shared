 ...
 */
export function buildExperimentLadder(
  goalType: BehaviorStateGoalType,
  customRungs: MilestoneRung[] | undefined,
  targetDays: number,
): MilestoneRung[] {
  const rungs = buildLadder(goalType, customRungs).filter(
    (r) => r.days <= targetDays,
  );
  ...
  if (rungs[rungs.length - 1].days !== targetDays) {
    rungs.push({
      days: targetDays,
      label: RUNG_LABELS[targetDays] ?? `${targetDays} days`,
    });
  }
  return rungs;
}
