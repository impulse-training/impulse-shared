import { getUnrecappedDays, DaySummaryLike, UnrecappedDay } from '../getUnrecappedDays';

function makeSummaries(ids: string[], flags?: Record<string, Partial<DaySummaryLike>>) {
...
describe('getUnrecappedDays (timezone-aware)', () => {
  test('excludes today, includes yesterday if not recapped (Australia/Adelaide +10:30)', () => {
    ...
    expect(res.find((r: UnrecappedDay) => r.dateString === '2025-10-19')).toBeTruthy();
    const y = res.find((r: UnrecappedDay) => r.dateString === '2025-10-19');
    expect(y?.label).toBe('Yesterday');

    // Today (2025-10-20) must not appear
    expect(res.find((r: UnrecappedDay) => r.dateString === '2025-10-20')).toBeFalsy();
  });
  ...
  test('clamps start to signup local day if later (America/Los_Angeles -07:00 DST)', () => {
    ...
    expect(res.find((r: UnrecappedDay) => r.dateString === '2025-07-12')).toBeFalsy();
    expect(res.find((r: UnrecappedDay) => r.dateString === '2025-07-13')).toBeTruthy();
  });
