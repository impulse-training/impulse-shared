...
function getGptPayload(log, isFinalLogInSession, options) {
    ...
    if ((0, log_1.logIsDebriefQuestionLog)(log)) {
        ...
        if (selected) {
            ...
            return [
                {
                    role: "user",
                    content: isFinalLogInSession && !(options === null || options === void 0 ? void 0 : options.forSummarization)
    ...
    if ((0, log_1.logIsTacticChoiceLog)(log)) {
        const [anchor, alternative] = log.data.options;
        const both = `"${anchor.title}" or "${alternative.title}"`;
        if (log.data.chosenTacticId) {
            const chosen = log.data.chosenTacticId === anchor.tacticId ? anchor : alternative;
            const notChosen = log.data.chosenTacticId === anchor.tacticId ? alternative : anchor;
        ...
        if (log.data.declinedBoth) {
            return [
                {
                    role: "user",
                    content: `<SYSTEM>You offered ${both}. The user wanted NEITHER. ` +
        ...
        return [
            {
                role: "user",
                content: `<SYSTEM>${both} are on screen right now as a choice, and the user has not answered yet. ` +
        ...
        if (tactics && tactics.length > 0) {
            ...
            if (isUserOwnedPlan) {
                ...
                return [
                    {
                        role: "user",
                        content: "<SYSTEM>" + opener + "Their own plan was assigned. " +
    ...
    if ((0, log_1.logIsWeekOverviewLog)(log)) {
        if (options === null || options === void 0 ? void 0 : options.forSummarization)
            return [];
        ...
        return [
            {
                role: "user",
                content: "<SYSTEM>The user was just shown their week card(s) for " +
